...
int main()
{
	...
	*/
	...
	/*num5 /= num6;                  -- так и не смог здесь починить
	std::cout << num5 << "\n";	*/
	...
	Нужно сделать меню, которое бы спрашивало число и присваивало ему определенный тип данных, который ему соответствует
	...
	*/
	...
	Deserialization<Point2d> d = { "TXT_Data.txt" };       // TXT_Data.txt  XML_Data.xml  JSON_Data.json
	std::cout << d.Deserialize();


	// добавить сюда исключения для типов
	
		//тут ошибка в создании объекта и не понимаю как её пофиксить
		// ошибка в том что необходимо указывать тип данных числа
		// При это не появляется ошибки что тип неправильный
	


	// добавить перед классами что это и что они делают
	// отрефакторить класс серилизации и прост добавить туда поля
